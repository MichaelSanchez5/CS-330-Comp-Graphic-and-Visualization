...
namespace
{
	...
	const char* const WINDOW_TITLE = "Michael_Sanchez_4-2 Assignment";
...
//input(load EXEfile from source mainframe)
int Exefile(GLFWindow* window)

void UResizeWindow(GLFWindow* window, int width, int height)
{
}

// Variables for window width and hieght
const int WINDOW_WIDTH = 1800;
const int WINDOW_HEIGHT = 1600;

struct GLMesh
{
	GLunit vao;   // Handle for the vortex array object
	Glunit vbo;   // Handle for the vortex buffer object
	Glunit nVertices;  // Number of indices of the mesh
};
...
float gLastY = WINODW_HEIGHT / 2.0f;
...
void UResizeWindow(GLFWindow* window, int width, int height);
void UProcessInput(GLFWindow* winodw);
void UMousePosstionCallback(GLFWindow* window, double xoffset, double yoffset);
viod UMouseButtonCallback(GLFWindow* window, int button, int action, int mods);

//Initailize GLFW,GFEW, and create a window
bool UInitialize(int argc, char* argv[], GLFWwindow** window)
{
	...
	* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
	...
	if (*window == NULL)
	{
		std::cout << "Failed to crate GFLW window" << std::endl;
		glfwTerminate();
		return false;
	}
	glfwMake ContextCurrent(*window);
	glfwSetFramebufferSizeCallback(*window, UResizeWinodw);
	glfwSetCursorPosCallback(*window, UMousePostionCallback);
	glfwSedtScrollCallback(*window, UMouseScrollCallabck);
	glfwSetButtonCallbcak(*window, UMouseButtonCallback);

	//tell GLFW to capture our mouse
	glfwSetInputMode(*window, GFLW_CURSOR, GLFW_CURSOR_DISABLLED);
	...
	// process all input: quary GFLW wether relevant keys are pressed/released this frame and react accordingly
	void UProcessInput(GLFWwindow * window)
	{
		...
		if (glfwGetgKey(window, GLFW_KEY_W) == GLFW_PRESS)
			gCamera.ProcessKeyboard(FORWARD, gDeltaTime);
		if (glfwGetKey(window, GLFW_HEY_S) == aGLFW_PRESS)
	...
	//glfw: whenevertghe window size changed (by OS or user resize) this callback funcitone excutes void UResizeWindow(GFLWwindow*window,int width,int heigth)
	{
		glViewport(0, 0, width, height);
	}

	// glfw: whenever the mouse moves, this callbck is called 
	//-------------------------------------------
	void UMousePostionCallBack(GFLWwindow * window, double xpos, double ypos)
	{
		if (gFirstMouse)
		{
			gLastX = xpos;
		...
		float yoffset = gLastY - ypos;//reversed since y-coordiantes go from bottom to top
	...
	//glfw: whenever the scroll wheel scrolls, this callback is called
	//---------------------------------------------------
	void UMouseScrollCallback(GLFWwindow * window, double xoffset, yoffset)
	{
		gCamera.ProcessMouseScroll(xoffest);
	}
	...
	void UMouseButtonCallback(GLFWwindow * window, int button, int action, int mods)
	{
		...
		{
			if (aciton == GLFW_PRESS)
				cout << "Left mouse button pressed" << endl;
			else
				cout << "Left mosue button releassed" << endl;
		}
		break;
		...
		case GLFW_MOUSE_BUTTON_MIDDLE:
		{
			if (action--GLFW_PRESS)
				cout << "Middle mosue button pressed" << endl;
			else
	...
	void display(void) {
		...
		glLoadIdenttity() ://Load the Identity Matrix to reset our drawing loctions 
			//glEnable(GL_DEPTH_TEST);

			glTranslatef(-1.0f, 0.0f, -5.0f);//Push everything 5 units back into the scene, otherwise we won't see the primitive
		glRotate(90, 1.0f, 0.0f, 0.0f);

		//currently yellow to tell the difference 
		glColor3f(0.7, 0.8, 0.1);
		glutSolidCylinder(1.0, 0.3, 200, 100);

		glFlush();//Flush the OpenGL buffersa into window

	}

	void reshape(int width, int height) {
		glViewport(0, 0, (GLsizei)width, (GLsizei)height);//Set our viewport to the size of our window
		glMatrixMode(GL_PROJECTION);//Switch to the projection matrix so we can manipulate how our scene is viewed
		glLoadIdentity();//Reset the projection matrix to the indentity martix so don't get any articfacts(cleaning up)

		gluPerspective(60, (GLfloat)width / (GLfloat)height, 1.0, 100.0);//Set the Field of view angle(in degrees),the asspect ratio of our window, and the new far planes

		glMatrixMode(GL_MODELVIEW);//Swicth back to the model view matrix, so that we can start drawing shapes correctly
	}

	int main(int argc, char* argv[]) {
		gluInit(&argc, argv);//Initialize GLUT
		//glutInitDisplayMode(GLUT_SINGLE);//Set up a basic display buffer (only sinlge buffer for now)
		if (!UInitialize(argc, argv, &gWindow))
			return EXIT_FAILURE;


		//glutInitWindowPostion(100,100);// Set the postion of the window

		glutDisplayfunc(display);// Tell GLUT to use the method "display" for rendering

		glutIdleFunc(dispaly);// Tell GLUT to use the method "diplay" as our idle method as well

		glutReshapeFunc(reshape);// Tell GLUT gto use the method "reshape" for reshaping 
		...
		while (!glfwWindowShouldClose(gWindow))
		{
			...
			gDeltaTime = currnetFrame - gLastFrame;
			gLastFrame = cuurnetFrame;
